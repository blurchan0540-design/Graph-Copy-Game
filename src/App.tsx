...
export default function App() {
  ...
  const calculateScore = () => {
    const target = currentLevel.targetBezier;
  ...
  if (gameState === 'start') {
    return (
      <div className="h-screen bg-surface flex items-center justify-center p-4 md:p-8 overflow-hidden">
        <div className="w-full max-w-5xl h-full max-h-[70vh] flex flex-col items-center justify-center border border-outline bg-surface text-on-surface font-body shadow-2xl text-center p-12 md:p-20 gap-12">
          ...
          <button 
            onClick={startGame}
            className="px-16 py-6 bg-primary text-on-primary font-bold uppercase tracking-[0.3em] text-xl hover:brightness-110 active:scale-95 transition-all"
          >
            Initialize
          </button>
        </div>
        ...
        <div className="w-full max-w-5xl h-full max-h-[85vh] flex flex-col border border-outline bg-surface text-on-surface font-body shadow-2xl overflow-hidden">
          ...
          <div className="flex-grow p-20 flex flex-col items-center justify-center gap-12">
            ...
            <div className="w-full max-w-2xl">
              <SimulationStage 
                referenceBezier={currentLevel.targetBezier}
            ...
            <div className="flex gap-2">
              <div className={cn("w-12 h-1 transition-colors", observationLoops >= 0 ? "bg-primary" : "bg-outline")} />
              <div className={cn("w-12 h-1 transition-colors", observationLoops >= 1 ? "bg-primary" : "bg-outline")} />
            </div>
          </div>
        </div>
      </div>
  ...
  if (gameState === 'play') {
    return (
      <div className="h-screen bg-surface flex items-center justify-center p-4 md:p-6 overflow-hidden">
        <div className="w-full max-w-4xl h-full max-h-[90vh] flex flex-col border border-outline bg-surface text-on-surface font-body shadow-2xl overflow-hidden">
          ...
          <footer className="h-20 px-8 border-t border-outline flex items-center justify-between bg-surface-container">
            ...
            <button 
              ...
              className="px-10 py-3.5 bg-primary text-on-primary font-bold uppercase tracking-widest text-xs hover:brightness-110 active:scale-95 transition-all"
            >
              Apply & Compare
            </button>
          </footer>
        ...
        <div className="w-full max-w-3xl h-full max-h-[75vh] flex flex-col border border-outline bg-surface text-on-surface font-body shadow-2xl overflow-hidden">
          ...
          <div className="flex-grow p-16 flex flex-col items-center justify-center text-center gap-8">
            <div className={cn(
              ...
              lastScore >= 90 ? "border-primary text-primary" : "border-outline text-on-surface-variant"
            )}>
              {lastScore >= 90 ? <Trophy size={48} /> : <RefreshCw size={48} />}
            </div>

            <div className="space-y-2">
              <h3 className="text-5xl font-black uppercase tracking-tighter">
                {lastScore >= 90 ? "Masterful!" : "Analyzed"}
              </h3>
              <p className="text-on-surface-variant font-mono text-xs uppercase tracking-widest">Round {roundScores.length} Accuracy</p>
            </div>

            <div className="text-8xl font-black font-mono text-primary">
              {lastScore}%
            </div>
  ...
  return null; // All states handled above
}
